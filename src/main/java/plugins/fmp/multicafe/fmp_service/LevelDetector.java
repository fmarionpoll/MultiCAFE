...
public class LevelDetector {
	...
	public void detectLevels(Experiment exp, BuildSeriesOptions options) {
		...
		// Ensure capillaries are synchronized with kymographs and cam data
		if (exp.getSeqCamData() != null && exp.getSeqCamData().getSequence() != null) {
			exp.getCapillaries().updateCapillariesFromSequence(exp.getSeqCamData().getSequence());
		}
		synchronizeCapillariesWithKymographs(exp, seqKymos);
		...
		for (Capillary capi : exp.getCapillaries().getList()) {
			...
			futures.add(processor.submit(new Runnable() {
				...
				public void run() {
					...
					processKymographImage(exp, seqKymos, imageKymo, tKymoFinal, capiFinal, 
							topLevelName, bottomLevelName, transformPass1, transformPass2, searchRect, jitter);
				}
			}));
		}
	...
	private void processKymographImage(Experiment exp, SequenceKymos seqKymos, IcyBufferedImage imageKymo, 
			int tKymo, Capillary capi, String topLevelName, String bottomLevelName,
			ImageTransformInterface transformPass1, ImageTransformInterface transformPass2,
			Rectangle searchRect, int jitter) {
		...
		// Store results - levels are stored in measurements.ptsTop and measurements.ptsBottom
		// Note: The actual level detection implementation needs to be restored from the original code
		...
		// Add ROI to sequence
		// Note: The actual implementation for converting Level2D to ROI and adding to sequence
		// needs to be restored from the original code. This is a placeholder.
		// The original code likely used a utility method to convert Level2D to ROI2DPolyLine
	}
