...
public class ExperimentTimeManager {
	...
	public void loadFileIntervalsFromSeqCamData(SequenceCamData seqCamData, String imagesDirectory) {
		if (seqCamData != null) {
			...
			lastImage_FileTime = seqCamData.getFileTimeFromStructuredName(seqCamData.getnTotalFrames() - 1);
			if (firstImage_FileTime != null && lastImage_FileTime != null) {
				...
				if (seqCamData.getnTotalFrames() > 1)
					camImageBin_ms = (camImageLast_ms - camImageFirst_ms) / (seqCamData.getnTotalFrames() - 1);
				else
					camImageBin_ms = 0;
	...
	public long[] build_MsTimeIntervalsArray_From_SeqCamData_FileNamesList(SequenceCamData seqCamData,
			long firstImage_ms) {
		camImages_ms = new long[seqCamData.getnTotalFrames()];
		for (int i = 0; i < seqCamData.getnTotalFrames(); i++) {
